                ...
                match x {
                    ...
                    _ => {
                        if x.is_ascii_alphabetic() {
                            ...
                            Err(anyhow!(LexerError::UnknwonChar(x)))
                        }
                    }
                }
            }
...
mod test_lexer {
    ...
    fn test_lexer() {
        ...
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Number));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::WhiteSpace));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Plus));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::WhiteSpace));
        assert!(lexer
        ...
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::End));
    }
    ...
    fn test_lexer_2() {
        ...
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Star));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::WhiteSpace));
        assert!(lexer
    ...
    fn test_lexer_3() {
        ...
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Slash));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::WhiteSpace));
        assert!(lexer
    ...
    fn test_lexer_4() {
        ...
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Literal));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::WhiteSpace));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Percent));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::WhiteSpace));
        assert!(lexer
        ...
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Hat));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Literal));
    ...
    fn test_lexer_6() {
        ...
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Prime));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Prime));
        assert!(lexer
    ...
    fn test_lexer_7() {
        ...
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Dot));
        assert!(lexer
            .next_token()
            .is_ok_and(|token| token.kind == TokenKind::Number));
        assert!(lexer
